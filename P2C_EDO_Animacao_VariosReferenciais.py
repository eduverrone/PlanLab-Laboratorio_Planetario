...
def EDO_P2C_CM(r, t, M1, M2):
    ...
    dx2 = Vx2
    dVx2 = -(G * (M1 + M2) * (x2 - x1) / mod_r12)
    dy2 = Vy1
    dVy2 = -(G * (M1 + M2) * (y2 - y1) / mod_r12)

    return array([dx2, dVx2, dy2, dVy2], float)
...
def p2c_EDO_animacao_variosreferenciais():
    ...
    R1 = [r[0] - X_CM, r[2] - Y_CM]
    R2 = [r[4] - X_CM, r[6] - Y_CM]
    ...
    menu(choices=['Escolha', 'CM', 'Corpo 1', 'Corpo 2'], index=0, bind=Cameras)
    scene.append_to_caption("\n\n    ")
    pausar=button(text="Rodar", bind=Rodar)
    scene.append_to_caption("\n\n    ")
    check1=checkbox(bind=Rastro1, text='Rastro 1', checked=False)
    scene.append_to_caption("\n    ")
    check2=checkbox(bind=Rastro2, text='Rastro 2', checked=False)
    scene.append_to_caption("\n    ")
    ...
    corpo1 = sphere(trail_color=color.orange, texture='sol.jpg', radius=int(3 * raio / 4), pos=vector(x0_1, y0_1, 0),
                    make_trail=False, interval=5, retain=200)
    ...
    corpo2 = sphere(color=vector(0.076, 0.733, 0.902), texture='sirius.jpg', radius=int(raio / 2),
                    pos=vector(x0_2, y0_2, 0), make_trail=False, interval=5, retain=200)
    ...
    while True:
        if running:
            rate(20)
            dr = passo_rk4(EDO_P2C_Origem, r, t, h, M1, M2)
            r = r + dr
            t = t + h
            ...
            if camval == "CM":
                ...
                # Corpo 01
                corpo1.pos = vector(R1[0], R1[1], 0)

                # Corpo 02
                corpo2.pos = vector(R2[0], R2[1], 0)
                aux = 1
                ...
                # Corpo 01
                corpo1.pos = vector(0, 0, 0)

                # Corpo 02
                corpo2.pos = vector(R[0], R[1], 0)
                ...
                # Corpo 02
                corpo2.pos = vector(0, 0, 0)

                # Corpo 01
                corpo1.pos = vector(R[0], R[1], 0)
